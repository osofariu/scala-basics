...
class V3_WithConverterSpec extends path.FunSpec with Matchers {

  describe("adding decorators to evaluators") {
    ...
    object JsonWriter {
      ...
      def write[T](value: T, converter: JsonConverter[T]): String = {
    ...
    val asJsonConverter = new JsonConverter[Expression] {
    ...
    object ExprToJsonFormatter {
      def toJson(expr: Expression): String = JsonWriter.write(expr, asJsonConverter)
    }
